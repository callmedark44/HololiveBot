...
# drop tags verified to yield no images under our hardcoded rating (scripts/bad_tags.json)
bl_file = os.path.join(BOT, "scripts", "bad_tags.json")
if os.path.exists(bl_file):
    bad = json.load(open(bl_file, encoding="utf-8"))
    for name, srcs in tags.items():
        for src, drop in bad.items():
            if src in srcs:
                srcs[src] -= set(drop)
